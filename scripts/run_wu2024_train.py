#!/usr/bin/env python
"""Wu2024 training script for in-domain training.

This script trains Wu2024's PointerNet from random initialization using the same
...
Usage:
    ...
    python scripts/run_wu2024_train.py --config configs/manhattan.yaml --total-steps 1000  # smoke test
"""
...
from src.train.wu2024_trainer import Wu2024Trainer, Wu2024TrainConfig
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train Wu2024 baseline")
    parser.add_argument(
        "--config",
        type=str,
...
def build_train_config(config: Dict[str, Any], args) -> Wu2024TrainConfig:
    """Build training config from YAML and CLI args."""
    train_cfg = config.get("train", {})
    wu2024_cfg = config.get("wu2024_train", config.get("eval", {}).get("wu2024", {}))
    
    return Wu2024TrainConfig(
        ...
        learning_rate=float(wu2024_cfg.get("learning_rate", 0.0005)),
        entropy_coef=float(wu2024_cfg.get("entropy_coef", 0.01)),
        value_loss_coef=float(wu2024_cfg.get("value_loss_coef", 0.5)),
        max_grad_norm=float(train_cfg.get("max_grad_norm", 5.0)),
        log_every_steps=int(train_cfg.get("log_every_steps", 1000)),
        eval_every_steps=int(wu2024_cfg.get("eval_every_steps", 10000)),
        ...
        kmax=int(wu2024_cfg.get("kmax", 32)),
        hidden_size=int(wu2024_cfg.get("hidden_size", 128)),
        num_layers=int(wu2024_cfg.get("num_layers", 1)),
        dropout=float(wu2024_cfg.get("dropout", 0.1)),
    )
...
def main():
    ...
    LOG.info("=" * 60)
    LOG.info("WU2024 IN-DOMAIN TRAINING")
    LOG.info("=" * 60)
    ...
    if args.run_dir:
        ...
        run_dir = PROJECT_ROOT / "reports" / "wu2024_train" / f"run_{timestamp}"
